...
Every release also publishes a **CycloneDX 1.5 SBOM**
(`singbox-vpn-sbom.cdx.json`) and a **license inventory**
(`singbox-vpn-licenses.json`), generated by `deploy/lib/generate-sbom.sh` in the
same build job from the tagged commit's `Cargo.lock` (`cargo metadata --locked`).
They cover the runtime dependency graph of `vpn-admin` and `subscription`
(registry checksums from `Cargo.lock`) plus the pinned upstream sing-box binary
the installer downloads. Generation fails if any runtime dependency lacks license
metadata. Both files are Sigstore-attested (`singbox-vpn-sbom.sigstore.json`) and
covered by `SHA256SUMS`; CI runs the generator on every change and
`deploy/lib/tests/test-release-sbom.sh` keeps the release workflow from dropping
them.
