...
echo "  Level-5/6 (an actual VLESS+REALITY handshake, and a real"
echo "  Hysteria2 transfer, through a throwaway sing-box client against"
echo "  this host's own live listeners) is deliberately NOT wired into"
echo "  this script — it already exists elsewhere and is not repeated"
echo "  here:"
echo "    sudo vpn-admin doctor --protocol --require-protocol"
echo "      # real sing-box client, live REALITY public_key/short_id,"
echo "      # full handshake + application bytes end-to-end (hard-fails"
echo "      # on a rejected handshake instead of just warning)."
echo "    sudo /opt/singbox-vpn/deploy/lib/vpn-benchmark.sh"
echo "      # real sing-box client transfer through BOTH the live"
echo "      # VLESS+REALITY and Hysteria2 listeners (throwaway user,"
echo "      # deleted afterward); reports throughput, not just PASS/FAIL."
echo "  Both are also exercised automatically by the destructive"
echo "  lifecycle gate: deploy/almalinux/lifecycle-acceptance.sh."
